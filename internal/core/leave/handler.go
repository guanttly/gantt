...
func (h *Handler) List(w http.ResponseWriter, r *http.Request) {
	...
	if !hasNodeLeaveAccess(r) {
		opts.EmployeeID = approle.CurrentEmployeeID(r.Context())
	}
...
func (h *Handler) Create(w http.ResponseWriter, r *http.Request) {
	...
	input.EmployeeID = approle.CurrentEmployeeID(r.Context())
...
func (h *Handler) ensureSelfLeave(w http.ResponseWriter, r *http.Request, leaveID string) bool {
	leave, err := h.svc.GetByID(r.Context(), leaveID)
	if err != nil {
		h.handleError(w, err)
		return false
	}
	if leave.EmployeeID != approle.CurrentEmployeeID(r.Context()) {
...
func hasNodeLeaveAccess(r *http.Request) bool {
	for _, permission := range approle.CurrentPermissions(r.Context()) {
		if permission == "leave:view:node" || permission == "leave:approve" {
			return true
		}
	}
	return false
