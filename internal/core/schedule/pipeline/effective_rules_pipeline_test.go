...
type pipelineOrgNodeResolver struct {
	nodes map[string]tenant.OrgNode
}

func (m *pipelineOrgNodeResolver) GetByID(_ context.Context, id string) (*tenant.OrgNode, error) {
	node, ok := m.nodes[id]
	if !ok {
...
type seedCandidatesStep struct{}

func (s *seedCandidatesStep) Name() string { return "SeedCandidates" }

func (s *seedCandidatesStep) Execute(_ context.Context, state *step.ScheduleState) error {
	state.Candidates["shift-night|2026-03-23"] = []string{"emp-alice", "emp-bob", "emp-charlie"}
	return nil
}

func setupPipelineServices(t *testing.T) (*rule.Service, *shift.Service, *gorm.DB, tenant.OrgNode) {
	t.Helper()
	...
	resolver := &pipelineOrgNodeResolver{nodes: map[string]tenant.OrgNode{root.ID: root, org.ID: org, dept.ID: dept}}
	ruleSvc := rule.NewService(rule.NewRepository(db), tenant.NewRepository(db))
	ruleSvc.SetOrgNodeResolver(resolver)
	shiftSvc := shift.NewService(shift.NewRepository(db))
	shiftSvc.SetOrgNodeResolver(resolver)

	return ruleSvc, shiftSvc, db, dept
}

func TestPipeline_Run_LoadsEffectiveRulesAndCompletesSchedule(t *testing.T) {
	ruleSvc, shiftSvc, db, dept := setupPipelineServices(t)
	ctx := tenant.WithOrgNode(context.Background(), dept.ID, dept.Path)

	seedStatements := []string{
		`INSERT INTO shifts (id, org_node_id, name, code, type, start_time, end_time, duration, is_cross_day, color, priority, status) VALUES ('shift-day', '` + dept.ID + `', 'Day', 'day', 'regular', '08:00', '16:00', 480, 0, '#111111', 1, 'active')`,
		`INSERT INTO shifts (id, org_node_id, name, code, type, start_time, end_time, duration, is_cross_day, color, priority, status) VALUES ('shift-night', '` + dept.ID + `', 'Night', 'night', 'regular', '20:00', '08:00', 720, 1, '#222222', 2, 'active')`,
...
`)
	...
	if _, err := ruleSvc.BatchCreateParsed(ctx, pipelineBatchInputFromParseResult(parsed)); err != nil {
		t.Fatalf("BatchCreateParsed() error = %v", err)
	}

	p := NewPipeline("effective-rules-integration", zap.NewNop(),
		&step.LoadRulesStep{RuleService: ruleSvc, ShiftService: shiftSvc},
		&seedCandidatesStep{},
		&step.PhaseZeroStep{},
		&step.PhaseOneStep{},
		&step.PhaseTwoStep{},
	...
	state := step.NewScheduleState("sch-1", dept.ID, "", "2026-03-22", "2026-03-23", "user-1", &step.ScheduleConfig{
	...
	if len(state.EffectiveRules) != 3 {
		t.Fatalf("expected 3 effective rules, got %d", len(state.EffectiveRules))
	}
	if len(state.ShiftOrder) != 2 || state.ShiftOrder[0].ID != "shift-day" || state.ShiftOrder[1].ID != "shift-night" {
		t.Fatalf("unexpected shift order: %+v", state.ShiftOrder)
	}
	...
	for _, assignment := range state.Assignments {
		...
		if assignment.ShiftID == "shift-night" && assignment.Date == "2026-03-23" {
			if assignment.EmployeeID != "emp-alice" {
				t.Fatalf("expected Alice on night shift, got %s", assignment.EmployeeID)
			}
			if assignment.Source != step.SourceFill {
				t.Fatalf("expected night assignment source %q, got %q", step.SourceFill, assignment.Source)
			}
			foundAliceNight = true
		}
	}
	if dayAssignments != 2 {
		t.Fatalf("expected 2 fixed day assignments, got %d", dayAssignments)
	}
	if !foundAliceNight {
		t.Fatal("missing Alice night assignment")
	}

	wantCandidates := []string{"emp-alice", "emp-bob"}
	gotCandidates := state.Candidates["shift-night|2026-03-23"]
	if len(gotCandidates) != len(wantCandidates) || gotCandidates[0] != wantCandidates[0] || gotCandidates[1] != wantCandidates[1] {
		t.Fatalf("night candidates mismatch, want %v, got %v", wantCandidates, gotCandidates)
	}
}

func pipelineBatchInputFromParseResult(result *ruleparse.ParseBatchResult) rule.BatchCreateInput {
	...
	for _, current := range result.Rules {
		parsedRules = append(parsedRules, rule.ParsedRuleInput{
			CreateInput: rule.CreateInput{
				...
				RuleData:        pipelineStringPtr(current.RuleData),
				Config:          current.Config,
				Priority:        current.Priority,
				SourceType:      current.SourceType,
				ParseConfidence: current.ParseConfidence,
				Version:         current.Version,
				Description:     pipelineStringPtr(current.Description),
			},
...
func pipelineStringPtr(value string) *string {
	if value == "" {
		return nil
	}
	copy := value
