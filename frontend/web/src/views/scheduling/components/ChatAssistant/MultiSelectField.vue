...
<template>
  <div class="multi-select-field">
    ...
    <div class="search-box">
      <el-input v-model="searchKeyword" :placeholder="placeholder" :prefix-icon="Search" clearable size="small" />
    </div>
    ...
    <div class="options-list">
      ...
      >
        <el-checkbox :disabled="option.disabled" :model-value="selectedValues.has(option.value)" @change="toggleOption(option.value)" @click.stop />
        <div class="option-content">
          ...
          <div v-if="option.extra" class="option-extra">
            ...
            <span v-if="option.extra.duration" class="duration-text">时长: {{ option.extra.duration }}分钟</span>
          </div>
        </div>
      </div>

      <div v-if="filteredOptions.length === 0" class="empty-state">
        <el-empty :image-size="80" description="没有找到匹配的选项" />
      </div>
    </div>
  </div>
</template>
