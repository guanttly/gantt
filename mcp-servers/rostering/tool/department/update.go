...
// updateDepartmentTool 更新部门工具
type updateDepartmentTool struct {
	logger   logging.ILogger
	provider service.IServiceProvider
}

func NewUpdateDepartmentTool(logger logging.ILogger, provider service.IServiceProvider) mcp.ITool {
	return &updateDepartmentTool{logger: logger, provider: provider}
}

func (t *updateDepartmentTool) Name() string {
	return "rostering.department.update"
}

func (t *updateDepartmentTool) Description() string {
	return "Update department information"
}

func (t *updateDepartmentTool) InputSchema() map[string]any {
...
func (t *updateDepartmentTool) Execute(ctx context.Context, input map[string]any) (*mcp.CallToolResult, error) {
	...
	dept, err := t.provider.Department().Update(ctx, id, req)
	if err != nil {
		t.logger.Error("Failed to update department", "id", id, "error", err)
		return common.NewExecuteError("Failed to update department", err)
	}
...
var _ mcp.ITool = (*updateDepartmentTool)(nil)
